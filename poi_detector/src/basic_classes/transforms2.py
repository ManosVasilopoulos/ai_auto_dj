...
class Transform:
    ...
    @staticmethod
    def get_stft_real(rate, data, win_time, fft_size):
    ...
    @staticmethod
    def get_stft_imaginary(rate, data, win_time, fft_size):
        # All wav files have a sample rate = 44100 Hz which means 44100 samples per second
        # Nx = 441 --> 10 ms of time
        # win_time-->window length in milliseconds
        one_ms = rate / 1000  # rate=44100 cycles per second. Which means 44.1 cycles per millisecond ==> one ms is represented as 44.1
    ...
    @staticmethod
    def get_real_im_stft(rate, data, win_time, fft_size):
        # All wav files have a sample rate = 44100 Hz which means 44100 samples per second
        # Nx = 441 --> 10 ms of time
        # win_time-->window length in milliseconds
        one_ms = rate / 1000  # rate=44100 cycles per second. Which means 44.1 cycles per millisecond ==> one ms is represented as 44.1
    ...
    @staticmethod
    def get_spectrogram(rate, data, win_time, fft_size):
        # All wav files have a sample rate = 44100 Hz which means 44100 samples per second
        # Nx = 441 --> 10 ms of time
        # win_time-->window length in milliseconds
        one_ms = rate / 1000  # rate=44100 cycles per second. Which means 44.1 cycles per millisecond ==> one ms is represented as 44.1
    ...
    @staticmethod
    def get_melspectrogram(rate, data, win_time, n_mels):
        one_ms = rate / 1000  # rate=44100 cycles per second. Which means 44.1 cycles per millisecond ==> one ms is represented as 44.1
    ...
    @staticmethod
    def get_constant_q(rate, data, n_cqt=84):
    ...
    def get_mfcc(self, rate, data, win_time, n_mfcc):
        melspectrogram = self.get_melspectrogram(rate, data, win_time, 256)
    ...
    def calculate_transform(self, samples, sample_rate, transform_type, n_feats, window_size,
        ...
        if transform_type == 'spectrogram':
            x = self.get_spectrogram(rate=sample_rate, data=samples, win_time=window_size, fft_size=n_feats)
        elif transform_type == 'melspectrogram':
            x = self.get_melspectrogram(rate=sample_rate, data=samples, win_time=window_size, n_mels=n_feats)
        elif transform_type == 'stft_real':
            x = self.get_stft_real(rate=sample_rate, data=samples, win_time=window_size, fft_size=n_feats)
        elif transform_type == 'stft_imaginary':
            x = self.get_stft_imaginary(rate=sample_rate, data=samples, win_time=window_size, fft_size=n_feats)
        elif transform_type == 'mfcc':
            x = self.get_mfcc(rate=sample_rate, data=samples, win_time=window_size, n_mfcc=n_feats)
            ...
            x = self.get_constant_q(rate=sample_rate, data=samples, n_cqt=n_feats)
        else:
            x_real, x_im = self.get_real_im_stft(rate=sample_rate, data=samples, win_time=window_size,
                                                   fft_size=n_feats)
            x = [x_real, x_im]
