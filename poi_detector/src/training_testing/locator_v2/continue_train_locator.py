...
for epoch in range(last_finished_epoch + 1, locator.max_epochs):
    print('-------------------------- Epoch', epoch, '--------------------------')
    for x_train, y_train in my_generator.train_flow(batch_size=batch_size * 3):
    ...
    for x_test, y_test in my_generator.test_flow(batch_size=batch_size * 3):
        if x_test.shape != (batch_size * 3, time_size, freq_size, 1):
            print('Skipped this training turn. x_train.shape:', x_test.shape)
            break
        if y_test.shape != (batch_size * 3, time_size):
            print('Skipped this training turn. y_train.shape:', y_test.shape)
            break
