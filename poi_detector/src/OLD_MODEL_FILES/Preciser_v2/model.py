...
from .test_balancer import TestPreciserBalancer
...
class Preciser(Architecture):
    ...
    def __init__(self, _id, batch_size, time_size, freq_size, transform_type, window_size):
        ...
        self.input_output_handler = PreciserBalancer(time_size, freq_size, transform_type, window_size)
        self.test_input_output_handler = TestPreciserBalancer(time_size, freq_size, transform_type, window_size)
    ...
    def test(self, x, y, epoch):
        history = self.cnn_model.evaluate(x, y, batch_size=1)
    ...
    def predict(self, x):
        predictions = self.cnn_model.predict(x, batch_size=1)
    ...
    def make_project_dir(self):
        new_project_dir = self.project_dir + 'Preciser v2/'
